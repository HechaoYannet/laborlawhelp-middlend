...
3. OpenHarness 作为编排层
OpenHarness 负责多轮会话、工具调用、MCP 接入和流式事件；middlend 负责把工具结果整理为前端可消费的数据结构。
...
3. `oh_lib_tool_policy=legal_minimal` 会把工具面收敛到 `skill`、`mcp__pkulaw__*`、`list_mcp_resources`、`read_mcp_resource` 与本地结构化工具（`labor_fact_extract`、`labor_compensation_calc`、`labor_document_gen`、`labor_lawyer_recommend`），降低模型在“工具过多”时直接文本回答的概率。
...
middlend 会在发给 OpenHarness 的 prompt 前附加中间件指令，核心要求包括：
