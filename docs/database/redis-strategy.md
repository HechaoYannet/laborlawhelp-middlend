...
| 键 | 类型 | TTL | 用途 |
|---|---|---|---|
| `session:{session_id}:lock` | 锁 | `session_lock_timeout_seconds`（默认 30s） | 会话级并发锁 |
| `session:{session_id}:stream:seq` | 字符串（计数器） | 无显式 TTL | `content_delta` 序号 |
| `rate:owner:{owner_id}:{YYYYMMDDHHMM}` | 字符串（计数器） | 60s | 每个 owner 每分钟限流计数 |
